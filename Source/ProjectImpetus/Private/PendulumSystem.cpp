...
const FVector UPendulumSystem::GetPendulumAnchorPosition3D()
{
	return FVector(anchor.X, -anchor.Y, 0.f);
}

const FVector2D UPendulumSystem::GetPendulumBobPosition2D()
{
	// -y since game setup quirk (up decreases y, down increases y)
	return FVector2D(playerBob.position.X, -playerBob.position.Y);
}

const FVector UPendulumSystem::GetPendulumBobPosition3D()
{
